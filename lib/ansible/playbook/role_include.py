...
class IncludeRole(TaskInclude):
    ...
    FROM_ARGS = ('tasks_from', 'vars_from', 'defaults_from',
                 'handlers_from')  # used to populate from dict in role
    ...
    def load(data, block=None, role=None, task_include=None, variable_manager=None, loader=None):
        ...
        if ir._role_name is None:
            raise AnsibleParserError(
                "'name' is a required field for %s." % ir.action, obj=data)

        if 'public' in ir.args and ir.action != 'include_role':
            raise AnsibleParserError(
                'Invalid options for %s: public' % ir.action, obj=data)

        # validate bad args, otherwise we silently ignore
        bad_opts = my_arg_names.difference(IncludeRole.VALID_ARGS)
        if bad_opts:
            raise AnsibleParserError('Invalid options for %s: %s' % (
                ir.action, ','.join(list(bad_opts))), obj=data)
        ...
        if apply_attrs and ir.action != 'include_role':
            raise AnsibleParserError(
                'Invalid options for %s: apply' % ir.action, obj=data)
    ...
    def copy(self, exclude_parent=False, exclude_tasks=False):
        ...
        new_me = super(IncludeRole, self).copy(
    ...
    def get_include_params(self):
        ...
        if self._parent_role:
            ...
            v.setdefault('ansible_parent_role_names', []).insert(
                0, self._parent_role.get_name())
            v.setdefault('ansible_parent_role_paths', []).insert(
                0, self._parent_role._role_path)
        return v
