...
def load_list_of_blocks(ds, play, parent_block=None, role=None, task_include=None, use_handlers=False, variable_manager=None, loader=None):
    ...
    if not isinstance(ds, (list, type(None))):
        raise AnsibleAssertionError(
            '%s should be a list or None but is %s' % (ds, type(ds)))
            ...
            if action in ('include', 'import_tasks', 'include_tasks'):
                ...
                if action == 'include_tasks':
                    ...
                    is_static = C.DEFAULT_TASK_INCLUDES_STATIC or \
                        ...
                        (not templar.is_template(
                            t.args['_raw_params']) and t.all_parents_static() and not t.loop)

                if is_static:
                    if t.loop is not None:
                        if action == 'import_tasks':
                            raise AnsibleParserError(
                                "You cannot use loops on 'import_tasks' statements. You should use 'include_tasks' instead.", obj=task_ds)
                        else:
                            raise AnsibleParserError(
                                "You cannot use 'static' on an include with a loop", obj=task_ds)
                    ...
                    while parent_include is not None:
                        ...
                        try:
                            ...
                            if not parent_include.statically_loaded:
                                raise AnsibleParserError(
                                    ...
                                    "or variables from inventory" % parent_include.args.get(
                                        '_raw_params'),
                                    obj=task_ds,
                        ...
                        if t._role:
                            new_basedir = os.path.join(
                                t._role._role_path, subdir, cumulative_path)
                    ...
                    try:
                        ...
                        if data is None:
                            display.warning(
                                'file %s is empty and had no tasks to include' % include_file)
                            continue
                ...
                if is_static:
                    if ir.loop is not None:
                        if action == 'import_role':
                            raise AnsibleParserError(
                                "You cannot use loops on 'import_role' statements. You should use 'include_role' instead.", obj=task_ds)
                        else:
                            raise AnsibleParserError(
                                "You cannot use 'static' on an include_role with a loop", obj=task_ds)
...
def load_list_of_roles(ds, play, current_role_path=None, variable_manager=None, loader=None, collection_search_list=None):
    ...
    if not isinstance(ds, list):
        raise AnsibleAssertionError(
            'ds (%s) should be a list but was a %s' % (ds, type(ds)))
