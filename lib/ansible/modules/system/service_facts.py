    ...
    def gather_services(self):
        ...
        if service_path is not None and chkconfig_path is None:
            rc, stdout, stderr = self.module.run_command(
                "%s --status-all 2>&1 | grep -E \"\\[ (\\+|\\-) \\]\"" % service_path, use_unsafe_shell=True)
            for line in stdout.split("\n"):
                ...
                services[service_name] = {
                    "name": service_name, "state": service_state, "source": "sysv"}
        ...
        if initctl_path is not None and chkconfig_path is None:
            ...
            for line in real_stdout.split("\n"):
                ...
                if m.group('pid'):
                    ...
                    pid = None  # NOQA
                payload = {"name": service_name, "state": service_state,
                           "goal": service_goal, "source": "upstart"}
        ...
        # RH sysvinit
            ...
            for line in stdout.split('\n'):
                ...
                if m:
                    ...
                    rc, stdout, stderr = self.module.run_command(
                        '%s %s status' % (service_path, service_name), use_unsafe_shell=True)
                    service_state = rc
                    if rc in (0,):
                        service_state = 'running'
                    # elif rc in (1,3):
                        ...
                        if 'root' in stderr or 'permission' in stderr.lower() or 'not in sudoers' in stderr.lower():
                            ...
                            service_state = 'stopped'
                    service_data = {"name": service_name, "state": service_state,
                                    "status": service_status, "source": "sysv"}
...
class SystemctlScanService(BaseService):
    ...
    def gather_services(self):
        ...
        if systemctl_path is None:
            return None
        rc, stdout, stderr = self.module.run_command(
            "%s list-units --no-pager --type service --all" % systemctl_path, use_unsafe_shell=True)
        for line in [svc_line for svc_line in stdout.split('\n') if '.service' in svc_line and 'not-found' not in svc_line]:
            ...
            if "running" in line:
                ...
                state_val = "stopped"
            services[service_name] = {
                "name": service_name, "state": state_val, "status": "unknown", "source": "systemd"}
        rc, stdout, stderr = self.module.run_command(
        ...
        for line in [svc_line for svc_line in stdout.split('\n') if '.service' in svc_line and 'not-found' not in svc_line]:
            try:
                ...
                self.module.fail_json(
                    msg="Malformed output discovered from systemd list-unit-files: {0}".format(line))
            if service_name not in services:
                services[service_name] = {
                    "name": service_name, "state": "unknown", "status": status_val, "source": "systemd"}
            else:
...
def main():
    ...
    if len(all_services) == 0:
        results = dict(
            skipped=True, msg="Failed to find any services. Sometimes this is due to insufficient privileges.")
    else:
