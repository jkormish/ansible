...
class ActionModule(ActionBase):
    ...
    def _get_module_args(self, fact_module, task_vars):
            ...
            if self._connection._load_name not in ('network_cli', 'httpapi', 'netconf'):
                ...
                if subset not in ('all', ['all']):
                    self._display.warning(
                        'Ignoring subset(%s) for %s' % (subset, fact_module))

            timeout = mod_args.pop('gather_timeout', None)
            if timeout is not None:
                self._display.warning(
                    'Ignoring timeout(%s) for %s' % (timeout, fact_module))
        ...
        mod_args = get_action_args_with_defaults(
            fact_module, mod_args, self._task.module_defaults, self._templar)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        modules = C.config.get_config_value(
            'FACTS_MODULES', variables=task_vars)
        ...
        if 'smart' in modules:
            ...
            network_os = self._task.args.get('network_os', task_vars.get(
                'ansible_network_os', task_vars.get('ansible_facts', {}).get('network_os')))
            modules.extend(
                [connection_map.get(network_os or self._connection._load_name, 'setup')])
        ...
        if parallel is False or (len(modules) == 1 and parallel is None):
            ...
            for fact_module in modules:
                ...
                res = self._execute_module(
                    module_name=fact_module, module_args=mod_args, task_vars=task_vars, wrap_async=False)
